...
foreach(res_name IN LISTS RESOURCE_NAMES)
    string(MAKE_C_IDENTIFIER "${res_name}" var_name)
    string(APPEND output "    target->registerImage(\"${res_name}\", ${var_name}_data, ${var_name}_size);\n")
endforeach()
string(APPEND output "}\n")
file(WRITE "${OUTPUT_FILE}" "${output}")
                ]=])

                set(_generated_images "${CMAKE_CURRENT_BINARY_DIR}/generated_images.h")
                add_custom_command(
                    OUTPUT "${_generated_images}"
                    COMMAND ${CMAKE_COMMAND}
                        "-DRESOURCE_FILES=${_img_res_files}"
                        "-DRESOURCE_NAMES=${_img_res_names}"
                        "-DOUTPUT_FILE=${_generated_images}"
                        -P "${_embed_script}"
                    DEPENDS ${_img_res_files}
                    VERBATIM
                )
                target_sources(${target}_APP PRIVATE
                    $<$<CONFIG:Release>:${_generated_images}>
                )
                target_include_directories(${target}_APP PRIVATE
                    $<$<CONFIG:Release>:${CMAKE_CURRENT_BINARY_DIR}>
                )
